...
- [okf.site —— 在线 validator / generator / converter](https://okf.site/) —— 社区工具站,在浏览器里直接校验 / 生成 / 转换 OKF bundle,免安装。
- [okf.md/faq —— OKF 常见问题](https://okf.md/faq/) —— 社区维护的 OKF FAQ 页,集中解答规范与用法常见疑问。
...
- [win4r/okf-handoff](https://github.com/win4r/okf-handoff) —— **Python**(纯 stdlib、零依赖,MIT,v0.1.0):把 OKF 当"长会话交接"格式用于 Claude Code——会话上下文将满时生成一个结构化 handoff(OKF 风格的 Markdown+YAML bundle,落在 git 仓库里),新会话据此安全续接。主打"验证优先":handoff 只记真实观测到的 git 状态与测试输出,续接会话先重新核验仓库再信任(*guidance, not ground truth*);OKF 只作设计模式、不作依赖,不接 Gemini / Google Cloud / 付费服务;含 e2e 测试 + GitHub Actions CI + `.claude/skills`/`commands` 集成。新增"agent 会话状态交接"场景,与 hermes-okf / echoes-vault 的持久记忆方向互补。
- [mattdav/okflint](https://github.com/mattdav/okflint) —— **Python**(已上 PyPI,MIT,v0.1.1):自比"文档库的 Ruff"的通用 OKF 合规 linter,确定性、可复现、无 LLM。两条命令——`audit`(库的清点与诊断:统计、坏链、待拆分候选,恒 `exit 0`)与 `validate`(规范合规门,合规 `exit 0` 否则 `exit 1`,供 pre-commit / CI 把关)。规则从 `okf-base.yaml` manifest 读取(类型 / 必填字段 / 状态词表可配置),区别于硬编码规则的 okf-lint / okf-conformance / okf-enforcer。
